...
/**
 * interrupt只是给线程设置一个中断标志位置，对正在运行的线程没有任何影响。 只有当线程在sleep、wait、join的时候，
 * 如果被interrupt，就会抛出InterruptedException，表示被中断当前的状态， 线程可以catch后，做操作或者直接结束线程。
...
public class InterruptTest implements Runnable {
	...
	public void run() {
		try {
			...
			System.out.println("第一次线程是否中断状态:" + Thread.currentThread().isInterrupted());
			Thread.sleep(10000L);
		...
		System.out.println("第二次线程是否中断状态:" + Thread.currentThread().isInterrupted());
